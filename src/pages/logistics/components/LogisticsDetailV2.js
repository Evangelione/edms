        ...
        {this.props.showMap ? <LogisticsMap/> :
          <Card bodyStyle={{transition: 'all 0.5s'}}
                ...
                title={<div><span
                  ...
                  style={{fontSize: 15, paddingLeft: 80, color: '#aaa'}}>创建时间：{currentLogistics.create_time}</span>
                  <div><span style={{color: '#545F76', fontSize: 15}}>对应订单编号：{currentLogistics.order_code}</span></div>
                </div>}
                extra={<Button size='large' onClick={this.showMap}
                               style={{
                                 background: '#FF9A74',
                                 borderColor: '#FF9A74',
                                 color: '#fff'
            ...
            <div style={{paddingLeft: 20, margin: '40px 0'}}>
              ...
              {currentLogistics.deliver_status === '0' ?
                <div>
                  ...
                  <div>
                    <div style={{color: '#A1A9B3', fontSize: 15, marginBottom: 20}}>司机没有及时接单的话，可以在这里操作帮他接单，保证订单能顺利进行~
                    </div>
                    <div style={{float: 'right', marginTop: '-60px', marginRight: 20}}>
                    ...
                    <div>
                      ...
                      <div style={{float: 'right', marginTop: '-63px', marginBottom: '40px', marginRight: 20}}>
                        <PoundModal title='上传装车磅单' load_num={currentLogistics.load_num}
                      ...
                      <div>
                        ...
                        <div style={{float: 'right', marginTop: '-63px', marginBottom: '40px', marginRight: 20}}>
                          <PoundModal title='上传卸车磅单' unload_num={currentLogistics.unload_num}
                                      unload_url={currentLogistics.unload_url} hidden={'unload'}
                        ...
                        <div>
                          ...
                          <div style={{float: 'right', marginTop: '-63px', marginBottom: '40px', marginRight: 20}}>
                            <PoundModal title='确认磅单' hidden='all' load_num={currentLogistics.load_num}
                                        load_url={currentLogistics.load_url}
                                        unload_num={currentLogistics.unload_num}
                                        unload_url={currentLogistics.unload_url}
                                        id={currentLogistics.id}
                                        load_time={currentLogistics.load_time}
                                        unload_time={currentLogistics.unload_time}>
                              <Button type='primary'
                                      style={{background: '#59C694', borderColor: '#59C694'}}>确认磅单</Button>
                            </PoundModal>
                          </div>
                        </div> : currentLogistics.deliver_status === '5' ?
                          <div>
                            <div style={{color: '#A1A9B3', fontSize: 15, marginBottom: 20}}>恭喜您，本订单已完成！您可以继续操作其它订单！
                            </div>
                            <div style={{float: 'right', marginTop: '-63px', marginBottom: '40px', marginRight: 20}}>
...
function mapStateToProps(state) {
  ...
  const {detailForm, companyOption, carOption, showMap} = state.logisticsDetail
  return {
    currentLogistics,
    detailForm,
    companyOption,
