...
export default {
  ...
  effects: {
    * fetch({payload: {page = 1, find_auth = '', find_str = ''}}, {call, put}) {
      const {data} = yield call(permissionService.getPermissionList, {page, find_auth, find_str})
      if (data.code === 1) {
        yield put({
          type: 'save', payload: {
            list: data.data.list,
    ...
    * postAvatar({payload: file}, {call, put}) {
      const {data} = yield call(permissionService.postAvatar, {file})
      if (data.code === 1) {
    ...
    * insertAdmin({payload: form}, {call, put}) {
      const {data} = yield call(permissionService.insertUser, {form})
    ...
    * modifyAdmin({payload: form}, {call, put}) {
      const {data} = yield call(permissionService.modifyUser, {form})
      if (data.code === -2) return false
      if (data.code === 1) {
        message.success(data.msg)
        yield put({type: 'save', payload: {currentTab: 'quanbu'}})
    ...
    * forbiddenControl({payload: id}, {call, put, select}) {
      const {data} = yield call(permissionService.forbiddenControl, {id})
      if (data.code === -2) return false
      if (data.code === 1) {
        message.success(data.msg)
